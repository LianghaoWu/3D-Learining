	...
	void OnGUI() {
		...
		if (state.getMessage() != "") {
			...
			if (GUI.RepeatButton (new Rect (10, 10, 100, 40), "Help")) {
				...
				if (GUI.Button (new Rect (castw (4.29f), casth (4f), width, height), "Priest On")) {
					action.priestSOnB();
				}
				if (GUI.Button (new Rect (castw (1.06f), casth (4f), width, height), "Devil On")) {
					action.devilEOnB();
				}
				if (GUI.Button (new Rect (castw (1.26f), casth (4f), width, height), "Priest On")) {
					action.priestEOnB();
				}
				if (GUI.Button (new Rect (castw (2.5f), casth (1.3f), width, height), "Left Off")) {
					action.offBoatL();
				}
				if (GUI.Button (new Rect (castw (1.6f), casth (1.3f), width, height), "Right Off")) {
					action.offBoatR();
				}
			}
		}
